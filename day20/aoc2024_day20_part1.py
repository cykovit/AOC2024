...
## Breadth First Search
    ...
    queue = [start_pos]  ## use list as queue
    while queue:
        current_pos = queue.pop(0)  ## dequeue first element
        for direction in (1, -1, 1j, -1j):  ## check all 4 cardinal directions
            neighbor = current_pos + direction
            if neighbor in trakc_pos and neighbor not in visited_nodes:
                visited_nodes[neighbor] = (current_pos, visited_nodes[current_pos][1] + 1)
                queue.append(neighbor)
    return visited_nodes

def calc_cheat(cheat_duration=2):
    cheat_savings = {}
